...
private:
     ...
     * this is the authoritative gameplay state
     * rendering ui and ai only ever see copies exported from it
     */
    struct UnitState {
        ...
        // these stay on the live unit so combat and movement do not need to look up the archetype every frame
        glm::vec3 position;
        float move_speed;
        float radius;
        float aggro_range;
        ...
        // active_order is the branch currently running
        // order_queue is deferred intent behind it
        bool moving;
        bool holding_position;
        std::deque<RtsOrder> order_queue;
        std::optional<RtsOrder> active_order;
        // cached world path generated by the pathfinder for the current positional goal
        std::vector<glm::vec3> path_points;
        std::size_t path_index;
        // goal cell cache lets movement know when a path can be reused versus rebuilt
        GridCoord goal_cell;
        bool has_goal_cell;
    };
     ...
     *
     * BuildingSystem owns the footprint occupancy details
     * this struct owns the team health construction and resource related state layered on top
     */
    struct OwnedBuildingState {
        // stable id shared with BuildingSystem
        std::uint32_t building_id;
        // owning team
        int team;
        // blueprint name used to recover archetype data later
        std::string archetype_id;
        // live health state
        float health;
        float max_health;
        // normalized zero to one construction progress
        float build_progress;
        // total time required to build from zero to one
        float build_time;
        // health restored per second by repair work
        float repair_rate;
        // whether the building is still in its incomplete construction phase
        bool under_construction;
        // cached original placement cost so cancel or refund paths do not need another archetype lookup
        std::vector<RtsResourceCost> cost;
    };

    // low level movement and target search helpers
    // these are the building blocks that the order state machine composes each frame
    ...
    // together these advance one unit from queued intent to active behavior every tick
    void pullNextQueuedOrder(UnitState& unit);
    void updateActiveOrder(UnitState& unit, float delta_seconds);

    // production helpers
    // production resolves separately from direct unit commands because buildings can finish work asynchronously
    ...
    // events are rebuilt every update tick and describe what happened during that tick only
